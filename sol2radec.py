...
"""
Short script to take in a SDF file and convert the TRK_SOL observations
to TRK_RADEC with the RA/Dec of the Sun set for the center of the observation.

Usage:
  sol2radec.py <input_SDF> <output_SDF>
...
def main(args):
	...
	# Check all of the observations for TRK_SOL
	...
	if not changed:
		print "No TRK_SOL observations found, nothing to be done."
		sys.exit()
	
	# Update the project office comments with this change
	if project.projectOffice.sessions[0] is None:
		project.projectOffice.sessions[0] = 'Shifted TRK_SOL to TRK_RADEC'
	else:
		project.projectOffice.sessions[0] += '; Shifted TRK_SOL to TRK_RADEC'
